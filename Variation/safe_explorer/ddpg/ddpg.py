...
class DDPG:
    def __init__(self,
        ...
        self._models = {
            ...
            'target_critic1': self._target_critic1,
            'target_critic2': self._target_critic2
        }
        ...
        self._policyDelay = 2
        self.std = 2
        self.noiseClip = self._config.action_noise_range * 10
    ...
    def _initialize_optimizers(self):
        ...
        self._critic1_optimizer = Adam(self._critic1.parameters(), lr=self._config.critic1_lr)
        self._critic2_optimizer = Adam(self._critic2.parameters(), lr=self._config.critic2_lr)
    ...
    def _get_target(self, batch):
        ...
        #action add gaussian noise to action and clip it
        #action += self._config.action_noise_range * torch.randn(self._env.action_space.shape)
        action += (self._config.action_noise_range * torch.normal(0,self.std,self._env.action_space.shape)).clamp(-self.noiseClip, self.noiseClip)
        action = (action).clamp(-self._env.action_space.high[0], self._env.action_space.high[0])


        #q = min(self._target_critic1(observation_next, action), self._target_critic2(observation_next, action))
        #q = tf.reduce_min(tf.stack([self._target_critic1(observation_next, action),self._target_critic2(observation_next, action)]),0).eval()
        q1 = np.minimum(self._target_critic1(observation_next, action).detach().numpy(), self._target_critic2(observation_next, action).detach().numpy())
        q = self._as_tensor(q1)
    ...
    def _update_batch(self):
        ...
        q_predicted1 = self._critic1(self._as_tensor(batch["observation"]),
                                   self._as_tensor(batch["action"]))
        # critic_loss = torch.mean((q_predicted.detach() - q_target) ** 2)
        # Seems to work better
        critic1_loss = F.smooth_l1_loss(q_predicted1, q_target)
        ...
        q_target2 = self._get_target(batch)
        q_predicted2 = self._critic2(self._as_tensor(batch["observation"]),
                                   self._as_tensor(batch["action"]))
        # critic_loss = torch.mean((q_predicted.detach() - q_target) ** 2)
        # Seems to work better
        critic2_loss = F.smooth_l1_loss(q_predicted2, q_target2)
        ...
        if True:
            ...
            actor_loss = -torch.mean(self._critic1(self._as_tensor(batch["observation"]), new_action))
            actor_loss.backward()
            self._actor_optimizer.step()

            # Update targets networks
            self._update_targets(self._target_actor, self._actor)
            self._update_targets(self._target_critic1, self._critic1)
            self._update_targets(self._target_critic2, self._critic2)

            # Log to tensorboard
            self._writer.add_scalar("critic loss", critic1_loss.item(), self._train_global_step)
    ...
    def evaluate(self):
        ...
        sum1 = np.sum(episode_rewards)

        self._writer.add_scalar("eval mean episode reward", mean_episode_reward, self._eval_global_step)
        self._writer.add_scalar("eval episode return", sum1, self._eval_global_step)
